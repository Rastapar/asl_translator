...
class ArmsLandmarkerHelperTest {
    ...
    fun landmarkerLiveStreamModeResultsFallsWithinAcceptedRange() {
        var landmarkerResult: ArmsLandmarkerHelper.ResultBundle? = null
        val armsLandmarkerHelper =
            ArmsLandmarkerHelper(
                context = ApplicationProvider.getApplicationContext(),
                runningMode = RunningMode.LIVE_STREAM,
                handLandmarkerHelperListener = object :
                    ArmsLandmarkerHelper.LandmarkerListener {
                    ...
                    override fun onResults(
                        resultBundle: ArmsLandmarkerHelper
                        .ResultBundle
                    ) {
        ...
        armsLandmarkerHelper.detectAsync(
            mpImage, SystemClock.uptimeMillis()
        )

        // Lock to wait the ArmsLandmarkerHelper return the value.
        lock.withLock {
            condition.await()
        }
    ...
    fun landmarkerVideoModeResultFallsWithinAcceptedRange() {
        val armsLandmarkerHelper = ArmsLandmarkerHelper(
            context = ApplicationProvider.getApplicationContext(),
        ...
        val landmarkerResult = armsLandmarkerHelper.detectVideoFile(
            videoUri,
            300
        )
    ...
    fun landmarkerImageModeResultFallsWithinAcceptedRange() {
        ...
        val landmarkerResults =
            armsLandmarkerHelper.detectImage(bitmap!!)?.results?.first()
